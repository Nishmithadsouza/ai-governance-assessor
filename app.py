...
from src.theme import ACCENTS, hero, inject_css, nav_card, stat_tiles

st.set_page_config(page_title="AI Governance Assessor", page_icon="🩺", layout="wide")
...
hero(
    ...
    f"{len(INDUSTRIES)} curated industries · {len(PRESET_INDUSTRIES)} preset industries · "
    "unlimited custom industries via the generic cross-industry baseline",
)

st.markdown(
    """
This application assesses AI use cases against a curated, citation-backed corpus of real
AI governance sources, across **9 governance dimensions**: Data Privacy, Bias/Fairness,
Human Oversight, Explainability, Security, Decision Impact, Regulatory Exposure, Model
Risk, and Monitoring — for whichever industry you select.
...
different analysis, not a relabeled copy of the same one. Two industries (Healthcare, HR/
Recruitment) have a full dedicated corpus. Every other industry — pick a preset, or **type any
industry name that doesn't exist in the dropdown at all** — is scored against a real, cited,
industry-agnostic baseline (NIST AI RMF, ISO/IEC 42001, OECD AI Principles, the UNESCO AI Ethics
Recommendation, the EU AI Act, Colorado's AI Act) instead of being refused or faked.
"""
)
